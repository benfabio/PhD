...
setwd(paste(WD,"/","raster_files_pAbs_simulation","/", sep= ""))
second.wd <- getwd()
...
# List of runs : 
times <- c(1:10)
# List of prevalences levels : 
prevalences <- c("n1","n1.5","n2","n10","n50")

### For testing function:
s <- "Calanus_helgolandicus.grd"
t <- 10
preval <- "n1.5"
...
pseudoAbsGenerator <- function(s) {
		...
		for(preval in prevalences) {
			...
			for(t in times) {
					...
					message(paste("Starting simulations of pseudoAbs for ",ss,"_",preval,"_run_",t, sep=""))
					...
					# Choose number of pAbs according to prevalences, with a classif if else loop
					if(preval == "n1") { 
								n <- 1
							} else if(preval == "n1.5") { 
								n <- 1.5
					...
					# dim(new.coords) ; head(new.coords) ; dim(your.coords) ; head(your.coords)
					...
					setwd(paste(second.wd,"/","pabs_files","/", sep=""))
		   			...
		   			if(t == 1) { 
							dir.create(ss)
							setwd(paste(second.wd,"/","pabs_files","/",ss,"/", sep="")) 
					   } else { 
							setwd(paste(second.wd,"/","pabs_files","/",ss,"/", sep="")) 
					} #eo ifelse
					
					# Save
					message(paste("Saving",ss,preval,"run",t, sep="_"))
					write.table(occurence.final, paste("pabs_",ss,"_",preval,"_run",t,".txt", sep = ""), sep="\t")
...
library("parallel")
mclapply(X = species_files, FUN = pseudoAbsGenerator, mc.cores = 20)
