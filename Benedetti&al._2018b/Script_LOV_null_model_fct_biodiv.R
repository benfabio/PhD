...
#### ISSUE HERE BECAUSE OF STR_SPLIT_FIXED
# Examine l. 24905 for instance
#ids$id[24905]
#str_split_fixed(ids$id[24905], "_", 2)
...
##### !!! COORDINATES
#coords <- get(load("med_coord.Rdata"))
#coords

### Read functional dendrogram:
setwd(WD)
dendro <- get(load("fct_dendro_corrected.Rdata"))
# class(dendro), cool object of class phylo.
# Change tip labels
dendro$tip.label <- str_replace_all(dendro$tip.label, "_", ".")
...
#d <- data.frame(t(resNullModel))
#colnames(d)[1:5] <- c("pval", "ses", "delta_fd", "fd_t0", "fd_t2")
#d$x <- coords$Lon
#d$y <- coords$Lat
...
#res <- lapply(files, function(f) {
			#d <- t(get(load(f)))
			#combination <- str_replace(f, "delta_FD_n", "")
			#combination <- str_replace(combination, ".Rdata", "")
			#colnames(d)[1:5] <- c(paste("pval", combination, sep ="_"), paste("ses", combination, sep ="_"), paste("delta", combination, sep= "_"), 
								  #paste("FDT0", combination, sep= "_"), paste("FDT2", combination, sep= "_") )
			#return(d)
#} ) # eo lapply

#d <- data.frame(do.call(cbind, res))
#dim(d)
#head(d)
#summary(d)
#class(d)
...
# save(d, file = "delta_FD_T2.Rdata")
# OR:
# d <- get(load("delta_FD_T2.Rdata"))

### Detail the structure of the matrix:
# - rows are geographic cells
# - there are 486 columns: 3*162
# - there are 3 values (p-value, ses & observed ∆FD) per combination of P/ENM/SCENARIO/CV RUN
#files
# For instance, first 3 columns are: 
# - p-value of n10_ANN_A1BARF_RUN1
# - ses of n10_ANN_A1BARF_RUN1
# - observed ∆FD of n10_ANN_A1BARF_RUN1

### Provide coordinates for mapping:
#coords <- data.frame(str_split_fixed( rownames(d) , "_", 2))
#colnames(coords)[1:2] <- c("Lon","Lat")
#d$x <- as.numeric(levels(coords$Lon))[coords$Lon]
#d$y <- as.numeric(levels(coords$Lat))[coords$Lat]
...
### Let's try a map then ? (use col n° 417: 'obs_50_MAXENT_A2ARF_RUN1')
#map <- ggplot() + geom_point(aes(x = x, y = y, colour = delta_fd), data = d) + coast + 
			#scale_colour_gradient2(name = "Modelled delta FD\n(2069-2098)\n", high = "red", low = "blue", midpoint= 0, guide = "colorbar") + 
			#coord_quickmap() + theme_bw()
			
#quartz()
#mapping
#ggsave("test_map_deltaFD.pdf", plot = map, width = 8, height = 6, dpi = 300)

# p-values mapping:
#map <- ggplot() + geom_point(aes(x = x, y = y, colour = pval), data = d) + 
			#scale_colour_distiller(name = "p-value\n", palette = "YlOrRd", guide = "colorbar", direction = 1, limits = c(0,0.05)) + 
			#coast + coord_quickmap() + theme_bw()
			
#quartz()
#map
#ggsave("test_map_pvalue.pdf", plot = map, width = 8, height = 6, dpi = 300)


#map <- ggplot() + geom_point(aes(x = x, y = y, colour = fd_t0), data = d) + coast +
			#scale_colour_distiller(name = "FD (T0)", palette = "YlOrRd", guide = "colorbar", limits = c(0, 100), direction = 1) + 
			#coord_quickmap() + theme_bw()

#ggsave("test_map_FD_T0.pdf", plot = map, width = 8, height = 6, dpi = 300)


#map <- ggplot() + geom_point(aes(x = x, y = y, colour = fd_t2), data = d) + coast +
			#scale_colour_distiller(name = "FD (T2)", palette = "YlOrRd", guide = "colorbar", limits = c(0, 100), direction = 1) + 
			#coord_quickmap() + theme_bw()

#ggsave("test_map_FD_T2.pdf", plot = map, width = 8, height = 6, dpi = 300)
